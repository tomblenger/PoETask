...
{
    return MapImageInternal( path, nullptr, 0, false, flags, mapCallback, context, pCustomArgs, moduleName, funcName );
}
    ...
    )
{
    ...
    return MapImageInternal( path, buffer, size, asImage, flags, mapCallback, context, pCustomArgs, moduleName, funcName );
}
    ...
    for (auto& img : _images)
    {
        ...
        if (!img->initialized)
        {
			...
			if (!img->peImage.pureIL() || img->peImage.isExe())
			{
				...
				if (flags & InjectShellCode)
				{
					auto pFunc = _process.modules().GetExport(_process.modules().GetModule(moduleName), funcName);
					procAddr = pFunc->procAddress;
				}
				status = RunModuleInitializers(img, DLL_PROCESS_ATTACH, pCustomArgs, flags, procAddr).status;
			}
...
call_result_t<uint64_t> MMap::RunModuleInitializers( ImageContextPtr pImage, DWORD dwReason, CustomArgs_t* pCustomArgs /*= nullptr*/, eLoadFlags flags /*= NoFlags*/, ptr_t procAddress /*= 0*/ )
{
    auto a = AsmFactory::GetAssembler( pImage->ldrEntry.type );
    uint64_t result = 0;
	...
	else
	{
		if (!_process.core().isWow64())
		{
			...
			(*a)->push(asmjit::host::rdx);
			(*a)->push(asmjit::host::r8);
			(*a)->push(asmjit::host::r9);
		}
	}
	...
	if (flags & InjectShellCode)
	{
		if (!_process.core().isWow64())
		{
			(*a)->pop(asmjit::host::r9);
			(*a)->pop(asmjit::host::r8);
			(*a)->pop(asmjit::host::rdx);
		...
		if (procAddress)
		{
			BYTE	bp = 0xCC;
			DWORD	oldProtect;
			BYTE pbOrginalCode[0x100];

			//auto b = AsmFactory::GetAssembler(pImage->ldrEntry.type);
			//(*b)->jmp(pCode->ptr<ptr_t>());
			//BLACKBONE_TRACE(L"Jmp address %016llX %d %016llX", pCode->ptr<ptr_t>(), (*b)->getCodeSize(), pImage->ldrEntry.entryPoint);
			//
			//_process.memory().Protect(procAddress, 0x1000, PAGE_EXECUTE_READWRITE, &oldProtect);
			//_process.memory().Write(procAddress, (*b)->getCodeSize(), (*b)->make());
			//_process.memory().Protect(procAddress, 0x1000, oldProtect);
			_process.memory().Protect(procAddress, 0x1000, PAGE_EXECUTE_READWRITE, &oldProtect);
			_process.memory().Read(procAddress, 0x100, pbOrginalCode);
			// Save the original code to temp buffer and then, restore in target process
			auto pCode = _process.memory().Allocate(0x100);
			pCode->Write(0, 0x100, pbOrginalCode);
			BLACKBONE_TRACE(L"Code size %d   Org Address = %016llX", (*a)->getCodeSize(), pCode->ptr<ptr_t>());
			_process.memory().Write(procAddress, (*a)->getCodeSize(), (*a)->make());
			_process.memory().Write(procAddress + (*a)->getCodeSize(), bp);
			_process.memory().Write(procAddress + (*a)->getCodeSize() + 1, pCode->ptr<ptr_t>());
			_process.memory().Protect(procAddress, 0x1000, oldProtect);
		}
	}
	else
