...
Session::Session() :socket(ios), writepacketlist(), strand_(ios)
{
	Init();
}
...
void Session::DoWrite(BOOL bStart)
{
	...
	socket.async_write_some(boost::asio::buffer(buffer+writeoffset, writepacket->used-writeoffset), 
		strand_.wrap(
	[this, self](boost::system::error_code ec, std::size_t sizewrite)
	{
		if(!ec)
		{
			...
			if(writeoffset == writepacket->used)
			{
				...
				{
					//DoDisconnect();
					strand_.post(boost::bind(&Session::DoDisconnect, this));
					return;
				}
				//DoWrite(TRUE);
				strand_.post(boost::bind(&Session::DoWrite, this, TRUE));
			}
			else
			{
				//DoWrite();
				strand_.post(boost::bind(&Session::DoWrite, this, FALSE));
			}
		}
		else
		{
...
void Session::DoReadBody(BOOL bStart)
{
	...
	if(bStart)
		readoffset = 0;
	socket.async_read_some(boost::asio::buffer(readpacket.bufferinternal+ REAL_BUFFER_OFFSET+readoffset, *(DWORD *)(readpacket.bufferinternal + 1)-readoffset), 
		strand_.wrap(
		[this, self](boost::system::error_code ec, std::size_t readsize)
	{
		if(!ec)
		{
			...
			{
				//DoReadBody();
				strand_.post(boost::bind(&Session::DoReadBody, this, FALSE));
			}
			else
			{
				...
				//DoReadHeader(TRUE);
				strand_.post(boost::bind(&Session::DoReadHeader, this, TRUE));
			}
		}
		else
		{
...
void Session::DoReadPatch( BOOL bStart)
{
	...
	if (bStart)
		readoffset = 0;
	socket.async_read_some(boost::asio::buffer(patch + REAL_BUFFER_OFFSET + readoffset, *(DWORD *)(patch + 1) - readoffset), 
	strand_.wrap(
		[this, self](boost::system::error_code ec, std::size_t readsize)
	{
		if (!ec)
		{
			...
			{
				//DoReadPatch();
				strand_.post(boost::bind(&Session::DoReadPatch, this, FALSE));
			}
			else
			{
				MakeEngineBuffer();
...
void Session::DoReadHeader(BOOL bStart)
{
	...
	if(bStart)
		readoffset = 0;
	socket.async_read_some(boost::asio::buffer(readpacket.bufferinternal+readoffset, REAL_BUFFER_OFFSET - readoffset), 
		strand_.wrap(
			[this, self](boost::system::error_code ec, std::size_t readsize)
	{
		if(!ec)
		{
			...
			{
				//DoReadHeader();
				strand_.post(boost::bind(&Session::DoReadHeader, this, FALSE));
			}
			else
			{
				...
				else */if (*(DWORD *)(readpacket.bufferinternal + 1) <= MAX_BUFFER_SIZE - REAL_BUFFER_OFFSET)
				{
					...
					{
						//DoReadBody(TRUE);
						strand_.post(boost::bind(&Session::DoReadBody, this, TRUE));
					}
				}
				else
				{
...
void	SendPatch(Session *session, BYTE *pbPatch)
{
	...
	session->strand_.post(boost::bind(&Session::DoWrite, session, TRUE));
}
